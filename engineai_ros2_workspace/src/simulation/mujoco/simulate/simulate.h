...
class Simulate {
 public:
  ...
  // Set a callback to be called after each render (for camera rendering, etc.)
  // The callback receives the model, data, and rendering context
  using PostRenderCallback = std::function<void(const mjModel*, const mjData*, mjrContext*)>;
  void SetPostRenderCallback(PostRenderCallback callback) { post_render_callback_ = callback; }

  // Get the rendering context (for camera rendering)
  mjrContext* GetRenderContext() { return &platform_ui->mjr_context(); }
  ...
  // Post-render callback (for camera rendering, etc.)
  PostRenderCallback post_render_callback_ = nullptr;
};
}  // namespace mujoco
